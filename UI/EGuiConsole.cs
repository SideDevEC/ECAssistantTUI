 ...
 _lastWidth = w;
    ...
    private void PaintOutputRegion()
    {
        ...
        for (int i = maxVisibleLine; i >= 0 && linesUsed < regionHeight; i--)
        {
            ...
            int visibleLen = AnsiHelpers.StripAnsi(line).Length;
            ...
            else
            {
                ...
                var wrapped = AnsiHelpers.WrapLine(line, _screenWidth);
    ...
    private void PaintStatusBar()
    {
        ...
        if (!string.IsNullOrEmpty(_statusBar))
        {
            ...
            int visibleLen = AnsiHelpers.StripAnsi(bar).Length;
            if (visibleLen > _screenWidth)
                bar = AnsiHelpers.TruncateAnsi(bar, _screenWidth);
            Console.Write(bar);
        }
    }
    ...
    public void PaintLayerScreen(string[] lines)
    {
        ...
        lock (_writeLock)
        {
            ...
            for (int i = 0; i < lines.Length && i < _screenHeight; i++)
            {
                ...
                int visibleLen = AnsiHelpers.StripAnsi(line).Length;
                if (visibleLen > _screenWidth)
                    line = AnsiHelpers.TruncateAnsi(line, _screenWidth);
